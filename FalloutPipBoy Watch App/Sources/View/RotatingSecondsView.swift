    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            TimelineView(.animation) { timeline in
                ...
                let secondFraction = Calendar.current.component(.second, from: now)
                let nanosec = Calendar.current.component(.nanosecond, from: now)
                let preciseSecond = Double(secondFraction) + Double(nanosec) / 1_000_000_000
                
                let minutes = Int(Calendar.current.component(.minute, from: now))
                ...
                ZStack {
                    ...
                    ForEach(0..<12) { second in
                        ...
                        let rotation = Angle.degrees(angle + Double(minutes) / 60.0 * 360.0)
                    ...
                    Path { path in
                        ...
                        path.addArc(center: CGPoint(x: geometry.size.width - arcOffset, y: center.y), radius: arcRadius, startAngle: .degrees(270), endAngle: .degrees(90), clockwise: true)
                    ...
                    ForEach(0..<12) { second in
                        let angle = Double(second + 16) / 12.0 * 360.0
                        let rotation = Angle.degrees(angle + preciseSecond / 60.0 * 360.0)
                    ...
                    HStack {
                        Group {
                            ...
                            Text(":\(Int(secondFraction))")
                        }
                        .font(.system(size: digitFontSize, weight: .bold, design: .rounded))
                        .foregroundColor(.red)
                    }
                    .position(x: geometry.size.width/2 - 0, y: geometry.size.height / 2 + size * 0.18)
                    #endif
